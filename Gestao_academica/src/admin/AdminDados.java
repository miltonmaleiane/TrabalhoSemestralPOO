...
public class AdminDados {
	...
	{	
		int result=0;
			try
			{
				String query="update admin set nomeEscola=?,endereco=?,emailid=?,numeroTelefone=?,website=?,lastlogin=?,password=?,logo=?,activestatus=?";
				PreparedStatement pr=con.prepareStatement(query);
				pr.setString(1, a.getNomeEscola());
				pr.setString(2, a.getEndereco());
				pr.setString(3, a.getEmailId());
				pr.setString(4, a.getNumero());
	...
	public Admin getAdminData()
	{
		...
		try
		{
			...
			ResultSet rs=st.executeQuery(query);
			rs.next();
			a.setNomeEscola(rs.getString(1));
			a.setEndereco(rs.getString(2));
			a.setEmailId(rs.getString(3));
			a.setNumero(rs.getString(4));
