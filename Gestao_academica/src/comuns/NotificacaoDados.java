...
public class NotificacaoDados {
	...
	public void addNotification(Notificacoes n)
	{
		try
		{
			...
			pr.setString(3, n.getCodigoCurso());
			pr.setString(5, n.getUserId());
			pr.setInt(4, n.getSemouano());
	...
	public ArrayList<Notificacoes> getNotifications(String courcecode,int semoryear,String userprofile,String joinedtime)
	{
		ArrayList<Notificacoes> list=new ArrayList<Notificacoes>();
		try
		{
			String query="select * from notificacao where courcecode='"+courcecode+"'  and semoryear="+semoryear+" and userprofile='"+userprofile+"' order by sr_no desc";
			...
			while(rs.next())
			{
				...
				n.setCodigoCurso(rs.getString(3));
				n.setSemouano(rs.getInt(4));
	...
	public void addReadBy(ArrayList<Integer> list,String userid)
	{
		try
		{
			String query="update notificacao set readby=concat(readby,?,'#') where ";
	...
	public int getUnreadNotification(String userid,String userprofile,String courcecode,int sem,String joinedtime)
	{
		...
		try
		{
			String query="select readby,time from notificacao where courcecode='"+courcecode+"' and semoryear="+sem+" and userprofile='"+userprofile+"'";
			Statement st=con.createStatement();
