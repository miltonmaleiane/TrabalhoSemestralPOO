...
import javax.swing.JscrollPainel;
...
public class PainelNotificacoes extends JPanel {
	...
	int row=200;
	String date="";
	ArrayList<Integer> readbylista=new ArrayList<Integer>();
	String latest="";
	private JPanel painelPrincipal;
	private JscrollPainel scrollPainel;
	private EstudanteMain sm;
	private DocenteMain fm;
	...
	public PainelNotificacoes() 
	{
		...
		JLabel headinglabel = new JLabel(" Notificações");
		...
		painelPrincipal.setSize(this.getWidth(), row);
		painelPrincipal.setLayout(null);
		painelPrincipal.setBackground(Color.white);
		
		 scrollPainel=new JscrollPainel(JscrollPainel.VERTICAL_SCROLLBAR_ALWAYS,JscrollPainel.HORIZONTAL_SCROLLBAR_NEVER);
		scrollPainel.setLocation(0, 188);
		scrollPainel.setSize(this.getWidth()-10,this.getHeight()-188);
		scrollPainel.setViewportView(painelPrincipal);
		scrollPainel.getVerticalScrollBar().setUnitIncrement(80);
		for(Component c:scrollPainel.getComponents())
		{
			c.setBackground(Color.white);
		}
		scrollPainel.setBorder(new EmptyBorder(0,0,0,0));
		
		add(scrollPainel);
		row=0;
	}
	public PainelNotificacoes(EstudanteMain sm)
	{
		this();
		this.sm=sm;
		addNotification(sm.s.getCodigoCurso(),sm.s.getSemouano(),"Student",sm.s.getIdusuario(),sm.s.getDataadmissao());
	}
	public PainelNotificacoes(DocenteMain fm)
	{
		this();
		this.fm=fm;
		addNotification(fm.f.getCodigoCurso(),fm.f.getSemouano(),"Faculty",fm.f.getFacultyId()+"",fm.f.getJoinedDate());
		
	}
	public void addNotification(String courcecode,int semoryear,String userprofile,String userid,String joinedtime)
	{
		ArrayList<Notificacoes> list=new NotificacaoDados().getNotifications(courcecode, semoryear,userprofile,joinedtime);
		for(Notificacoes n:list)
		{
		...
		if(readbylista.size()>0)
		{
		new NotificacaoDados().addReadBy(readbylista, userid);
		}
	...
	public JPanel createPanel(Notificacoes n)
	{
		...
		{
			image=new DadosEstudante().getProfilePic(n.getUserId()).getScaledInstance(65, 65, Image.SCALE_SMOOTH);
		}
		else if(n.getTitle().equals("Subject Faculty"))
		{
			image=new DadosDocente().getProfilePic(n.getUserId()).getScaledInstance(65, 65, Image.SCALE_SMOOTH);;
		}
		else
		{
			image=new AdminDados().getProfilePic().getScaledInstance(65, 65, Image.SCALE_SMOOTH);;
		}
						...
						if(e.getButton()==MouseEvent.BUTTON1&&e.getClickCount()>0)
						{
							...
							if(title.equals("New Student"))
							{
								...
								if(sm!=null)
								{
									...
									sm.viewstudentpanel=new PainelVerEstudante(s,sm,sm.notificationpanel);
								...
								{
									fm.viewstudentpanel=new PainelVerEstudante(s,fm,fm.notificationpanel);
							...
							{
								Docente f=new DadosDocente().getFacultyInfobyId(Integer.parseInt(userid));
								if(sm!=null)
								{
									sm.viewfacultypanel=new PainelVerDocente(f,sm,sm.notificationpanel);
								...
								{
									fm.viewfacultypanel=new PainelVerDocente(f,fm,fm.notificationpanel);
							...
							else if(title.equals("Result"))
							{
								...
								{
									sm.notificationpanel.setVisible(false);
									PainelNotas marksheetpanel=new PainelNotas(sm,sm.s,sm.notificationpanel);
									marksheetpanel.setVisible(true);
									sm.marksheetpanelscroll=new JscrollPainel(marksheetpanel,JscrollPainel.VERTICAL_SCROLLBAR_ALWAYS,JscrollPainel.HORIZONTAL_SCROLLBAR_NEVER);
								...
								{
									fm.notificationpanel.setVisible(false);
									fm.studentpanel=new PainelEstudante(fm,fm.notificationpanel);
							...
							else if(title.equals("Attandance"))
							{
								...
								{
									sm.notificationpanel.setVisible(false);
									sm.attandancereportpanel=new PainelVerFaltas(sm,sm.notificationpanel);
									sm.attandancereportpanel.setVisible(true);
									sm.attandancereportpanelscroll=new JscrollPainel(sm.attandancereportpanel,JscrollPainel.VERTICAL_SCROLLBAR_ALWAYS,JscrollPainel.HORIZONTAL_SCROLLBAR_NEVER);
							...
							else if(title.equals("New Subject"))
							{
								...
								{
									sm.notificationpanel.setVisible(false);
									sm.subjectpanel=new PainelDisciplina(sm,sm.notificationpanel);
								...
								{
									fm.notificationpanel.setVisible(false);
									fm.subjectpanel=new PainelDisciplina(fm,fm.notificationpanel);
