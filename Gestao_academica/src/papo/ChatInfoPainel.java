...
public class ChatInfoPainel extends JPanel {
	...
	public void setStudent(Estudante s)
	{
		this.student=s;
		headerlabel.setText("Your Info");
	}
	public void setFaculty(Docente f)
	{
		this.faculty=f;
		headerlabel.setText("Your Info");
	}
	...
	public void setData(Estudante s)
	{
		this.Enable();
		if(student!=null && s.getIdusuario().equals(student.getIdusuario()))
		{
		...
		bottomlabel.setText(s.getIdusuario());
		contactnamelabel.setText(s.getNomecompleto());
		...
		contactnumberlabel.setText("+258- "+s.getContactNumber());
	...
	public void setData(Docente f)
	{
		...
		bottomlabel.setText("("+f.getFacultyId()+") "+f.getCodigoCurso()+"-"+f.getSemouano()+"-"+f.getSubject());
	...
	public void setData(Admin a)
	{
		...
		contactnumberlabel.setText(a.getNumero());
	...
	private void membersPanel(Group g)
	{
		...
		{
			JPanel studentpanel=createPanel(student.getProfilePic(),"You","Student-"+student.getIdusuario(),true);
		...
		if(gn.contains("Official"))
		{
			if(admin==null)
			{
				JPanel adminpanel=createPanel(g.getImage(),"Principal","Admin",new AdminDados().getAdminData().getActiveStatus());
				memberspanel.add(adminpanel);
				total++;
			}
	...
	for(ContactInfo c:PainelListaContactos.contactinfo)
		{
			
		
			if((gn.contains("Students")||gn.contains("Official"))&&c.getClassName().equals("Student"))
			{
				Estudante s=c.getStudent();
				if(s.getCodigoCurso().equals(g.getCourceCode())&&s.getSemouano()==g.getSemorYear())
				{
				JPanel panel=createPanel(s.getProfilePic(),s.getNomecompleto(),"Student-"+s.getIdusuario(),s.getActiveStatus());
			...
			{
				Docente f=c.getFaculty();
				if(f.getCodigoCurso().equals(g.getCourceCode())&&f.getSemouano()==g.getSemorYear())
				{
