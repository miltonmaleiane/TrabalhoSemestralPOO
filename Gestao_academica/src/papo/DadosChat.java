...
public class DadosChat {
	...
	public int saveMessage(ChatUsuario u)
	{
		int result=0;
		try 
		{
	...
	public ArrayList<ChatUsuario> getMessages()
	{
		ArrayList <ChatUsuario> list=new ArrayList<ChatUsuario>();
		try 
		{
			...
			while(rs.next())
			{
				ChatUsuario user=new ChatUsuario();
	...
	public ArrayList<ChatUsuario> getUserMessages(ChatUsuario u)
	{
		ArrayList <ChatUsuario> list=new ArrayList<ChatUsuario>();
		try 
		{
	...
	public int getUndreadMessageCountStudent(Estudante s)
	{
		int total=0;
		String cource=s.getCodigoCurso()+" "+new DadosCurso().getsemoryear(s.getCodigoCurso())+"-"+s.getSemouano();
		try
		{
			String query="select readby from chat where ((touserid='"+cource+" Official Group' or touserid='"+cource+" Students Group') and fromuserid!='"+s.getIdusuario()+"') or touserid='"+s.getIdusuario()+"'";
			Statement st=con.createStatement();
			ResultSet rs=st.executeQuery(query);
			while(rs.next())
			{
				...
				boolean isreaded=isReadBy(readbystr,s.getIdusuario());
				if(!isreaded)
				{
					total++;
				}
	...
	public int getUndreadMessageCountFaculty(Docente f)
	{
		int total=0;
		String cource=f.getCodigoCurso()+" "+new DadosCurso().getsemoryear(f.getCodigoCurso())+"-"+f.getSemouano();
		try
		{
