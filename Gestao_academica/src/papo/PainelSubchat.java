...
public class PainelSubchat extends JPanel implements ActionListener,Runnable {

	Box vertical = Box.createVerticalBox();
	private JTextField messagefield;
	private ChatUsuario user=null;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					PainelSubchat frame = new PainelSubchat(null);
	...
	public PainelSubchat(ChatUsuario user) {
	...
	{
			ArrayList<ChatUsuario> list=new DadosChat().getUserMessages(user);
			...
			EventQueue.invokeLater(new Runnable()
			{
				...
				public void run() 
				{
					...
					{
					new DadosChat().addReadBy(readbylist,user.getFromUserId());
					}
				}
	...
	public  void sendmessage()
	{
		if(socket!=null)
		{
			try
			{
				...
				user.setSr_no(new DadosChat().getNewSr_no());
				new DadosChat().saveMessage(this.user);
	...
	public void RightSidePanel(ChatUsuario u)
	{
	...
	public void	LeftSidePanel(ChatUsuario u)
	{
	...
	public JPanel formatPanel(ChatUsuario u,boolean from)
	{
		...
		if(from)
		{
			...
			if(!user.getToUserId().contains("Group"))
			{
				...
				else
				{
					...
					{
						if(new DadosEstudante().isActive(user.getToUserId()))
						{
					...
					{
						if(new DadosDocente().isActive(user.getToUserId()))
						{
							timelabel.setIcon(new ImageIcon("./assets/doubletick.png"));
						}
					}
	...
	public void run() 
	{
		...
		{
					ChatUsuario u=null;
					while(socket!=null&&!socket.isClosed()&&(u=(ChatUsuario)reader.readObject())!=null)
					{
						if(u.getFromUserId().equals(user.getFromUserId())&&u.getToUserId().equals(user.getToUserId()))
						{
						...
						{
							new DadosChat().readBy(u.getSr_no(), u.getReadBy()+user.getFromUserId()+"#");
	...
	class MessageStatusActionListener implements ActionListener
	{
		...
		public void actionPerformed(ActionEvent arg0) 
		{
			for(int i=0; i<messagestatus.size(); i++)
			{
					...
					StringTokenizer read=new StringTokenizer(new DadosChat().getReadBy(m.getSrNo()),"#");
					boolean contain=false;
					while(read.hasMoreTokens())
					{
...
class MessageStatus
{
	...
	public MessageStatus(ChatUsuario u,JLabel label)
	{
		sr_no=u.getSr_no();
		this.label=label;
	}
